...
int Last_Encoder=10000;//上一次编码器的值
int D_Encoder;//编码器值的差
u8 any_position=0;//随机位置标志符
/**************************************************************************
函数功能：所有的控制代码都在这里面
          TIM1控制的5ms定时中断 
**************************************************************************/
...
{    
	...
	{   
      ...
      Angle_Balance=Get_Adc_Average(3,15);                     //===更新姿态			 
			if(any_position==0) any_position=1,Position_Zero= Encoder; //===任意位置顺摆
			...
			//电机飞转保护 
			D_Encoder=Encoder-Last_Encoder;
			Last_Encoder=Encoder;
			if(myabs(D_Encoder)>10)
			{
				Set_Pwm(0);
				Flag_Stop=1;
			}
		  ...
		  if(Turn_Off(Voltage)==0)              //===低压和倾角过大保护
			...
			else 	Flag_Stop=1,any_position=0;
...
{
	    u8 temp; 
			if(1==Flag_Stop||voltage<1110) //电池电压过低，关闭电机
			{	      
      Flag_Stop=1;				
      temp=1;                                            
			AIN1=0;                                            
