...
float Vol;
extern float theta, theta_dot, alpha, alpha_dot, last_alpha;//旋转臂的转角，旋转臂的角速度，摆杆的倾角，摆杆的角速度
/**************************************************************************
函数功能：OLED显示
入口参数：无
返回  值：无
...
void oled_show(void)
{
	//=============第1行当前滑块位移=======================//	:
	 ...
	 if(alpha<0)
	 {
		 ...
		 OLED_ShowNumber(54,0,-(int)alpha,1,12); //显示整数部分
	   OLED_ShowString(62,0,".");        //显示小数点
     OLED_ShowNumber(68,0,-(alpha-(int)alpha)*100,2,12); //显示整数部分
     OLED_ShowString(92,0,"rad");           //显示单位		
     if(-(alpha-(int)alpha)*100<10)
		 {
...
//	 OLED_ShowChinese_12(0,10,110,1);
   ...
   OLED_ShowChinese_12(24,10,113,1);
	 ...
	 if(alpha_dot<0)
	 {
		 ...
		 OLED_ShowNumber(54,10,-(int)alpha_dot,1,12); //显示整数部分
	   OLED_ShowString(62,10,".");        //显示小数点
     OLED_ShowNumber(68,10,-(alpha_dot-(int)alpha_dot)*100,2,12); //显示整数部分
     OLED_ShowString(84,10,"rad/s");           //显示单位		
     if(-(alpha_dot-(int)alpha_dot)*100<10)
		 {
	 ...
	 if(theta<0)
	 {
		 ...
		 OLED_ShowNumber(54,20,-(int)theta,1,12); //显示整数部分
	   OLED_ShowString(62,20,".");        //显示小数点
     OLED_ShowNumber(68,20,-(theta-(int)theta)*100,2,12); //显示整数部分
     OLED_ShowString(92,20,"rad");           //显示单位		
     if(-(theta-(int)theta)*100<10)
		 {
	 ...
	 if(theta_dot<0)
	 {
		 ...
		 OLED_ShowNumber(54,30,-(int)theta_dot,1,12); //显示整数部分
	   OLED_ShowString(62,30,".");        //显示小数点
     OLED_ShowNumber(68,30,-(theta_dot-(int)theta_dot)*100,2,12); //显示整数部分
     OLED_ShowString(84,30,"rad/s");           //显示单位		
     if(-(theta_dot-(int)theta_dot)*100<10)
		 {
