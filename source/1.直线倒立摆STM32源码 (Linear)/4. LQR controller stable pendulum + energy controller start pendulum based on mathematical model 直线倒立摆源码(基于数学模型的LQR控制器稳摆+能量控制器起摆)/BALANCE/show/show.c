...
void oled_show(void)
{
	 ...
	 OLED_ShowString(36,0,":");  //显示信息
	
	 if(x<0)
	 {
		 OLED_ShowString(46,0,"-");
		 OLED_ShowNumber(54,0,-(int)x,1,12); //显示整数部分
	   OLED_ShowString(62,0,".");        //显示小数点
     OLED_ShowNumber(68,0,-(x-(int)x)*100,2,12); //显示整数部分
     OLED_ShowString(92,0,"m");           //显示单位		
     if(-(x-(int)x)*100<10)
		 {
			 OLED_ShowNumber(68,0,0,1,12); //显示整数部分
		 }			 
	 }
	 else
	 {
     ...
     if((x-(int)x)*100<10)
		...
		//=============第2行滑块的线速度=======================//	
	 OLED_ShowChinese_12(0,10,110,1);
	 OLED_ShowChinese_12(8,10-2,111,1);
	 OLED_ShowChinese_12(16,10,112,1);
   OLED_ShowChinese_12(24,10,113,1);
	 OLED_ShowChinese_12(32,10,114,1);
	 OLED_ShowString(36,10,":");         //显示信息
	 
	 if(x_speed<0)
	 {
		 OLED_ShowString(46,10,"-");
		 OLED_ShowNumber(54,10,-(int)x_speed,1,12); //显示整数部分
	   OLED_ShowString(62,10,".");        //显示小数点
     OLED_ShowNumber(68,10,-(x_speed-(int)x_speed)*100,2,12); //显示整数部分
     OLED_ShowString(92,10,"m/s");           //显示单位		
     if(-(x_speed-(int)x_speed)*100<10)
		 {
			 OLED_ShowNumber(68,10,0,1,12); //显示整数部分
		 }			 
	 }
	 else
	 {
     ...
     if((x_speed-(int)x_speed)*100<10)
		...
		//=============第3行显示角度=======================//	
	 OLED_ShowChinese_12(0,20+1,108,1);
	 OLED_ShowString(36,20,":");            //显示信息
	 
	 if(angle<0)
	 {
		 OLED_ShowString(46,20,"-");
		 OLED_ShowNumber(54,20,-(int)angle,1,12); //显示整数部分
	   OLED_ShowString(62,20,".");        //显示小数点
     OLED_ShowNumber(68,20,-(angle-(int)angle)*100,2,12); //显示整数部分
     OLED_ShowString(92,20,"rad");           //显示单位		
     if(-(angle-(int)angle)*100<10)
		 {
			 OLED_ShowNumber(68,20,0,1,12); //显示整数部分
		 }			 
	 }
	 else
	 {
     ...
     if((angle-(int)angle)*100<10)
		...
		//=============第4行显示角速度=======================//	
	 OLED_ShowChinese_12(0,30+1,108,1);
	 OLED_ShowChinese_12(8,30-2,111,1);
	 OLED_ShowChinese_12(16,30,112,1);
	 OLED_ShowChinese_12(24,30,113,1);
	 OLED_ShowChinese_12(32,30,114,1);
	 OLED_ShowString(36,30,":");           //显示信息

	 if(angle_speed<0)
	 {
		 OLED_ShowString(46,30,"-");
		 OLED_ShowNumber(54,30,-(int)angle_speed,1,12); //显示整数部分
	   OLED_ShowString(62,30,".");        //显示小数点
     OLED_ShowNumber(68,30,-(angle_speed-(int)angle_speed)*100,2,12); //显示整数部分
     OLED_ShowString(84,30,"rad/s");           //显示单位		
     if(-(angle_speed-(int)angle_speed)*100<10)
		 {
			 OLED_ShowNumber(68,30,0,1,12); //显示整数部分
		 }			 
	 }
	 else
	 {
...
// Page 1: Loosen the top screw connecting the pendulum and the angular displacement sensor, and then adjust the knob located in the middle of the three screws 
void step_1(void)
{
...
// Page 2: Make the ADC value between 1010 and 1030 when the pendulum is vertically down, and then tighten the topmost screw. tune 
void step_2(void)
{
...
//第3页：节完成后按下用户按键开始检查
// Page 3: After the section is complete, press the user button to start the inspection 
void step_3(void)
{
...
// System Check Page: Checking for Correct Adjustment........10 Please keep the pendulum still 
...
// Adjustment success page : Adjustment is successful Press the user button to enter the system
...
//调节失败页：	检查不通过  请重新调节
// Adjustment failed page: Check failed, please readjust 
